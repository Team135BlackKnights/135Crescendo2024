...
public class OutakeS extends SubsystemBase {
    ...
    public static RelativeEncoder topFlywheelEncoder,
    ...
    private final static LinearSystem<N1, N1, N1> m_topFlywheelPlant =
            ...
            LinearSystemId.identifyVelocitySystem(Constants.OutakeConstants.kVVoltSecondsPerRotation * .995,
    ...
    // for easy control.
    private final static LinearSystemLoop<N1, N1, N1> m_topLoop = new LinearSystemLoop<>(m_topFlywheelPlant,
            m_topController, m_topObserver, 12.0, 0.020); // max physical voltage, not applied.
    ...
    private final static LinearSystem<N1, N1, N1> m_bottomFlywheelPlant = // LinearSystemId.createFlywheelSystem(DCMotor.getNEO(1),
            ...
            LinearSystemId.identifyVelocitySystem(Constants.OutakeConstants.kVVoltSecondsPerRotation * .995,
                    Constants.OutakeConstants.kAVoltSecondsSquaredPerRotation);
    private final static KalmanFilter<N1, N1, N1> m_bottomObserver = new KalmanFilter<>(Nat.N1(), Nat.N1(),
            m_bottomFlywheelPlant, VecBuilder.fill(3.0), VecBuilder.fill(0.01), .02);
    private final static LinearQuadraticRegulator<N1, N1, N1> m_bottomController = new LinearQuadraticRegulator<>(
            m_bottomFlywheelPlant, VecBuilder.fill(1), VecBuilder.fill(12.0), 0.020);
    private final static LinearSystemLoop<N1, N1, N1> m_bottomLoop = new LinearSystemLoop<>(m_bottomFlywheelPlant,
            m_bottomController, m_bottomObserver, 12.0, 0.020);
    private double topNextVoltage, bottomNextVoltage;
    private final FlywheelSim topFlywheelSim = new FlywheelSim(m_topFlywheelPlant, DCMotor.getNEO(1),
            1 / Constants.OutakeConstants.flywheelGearRatio);
    private final FlywheelSim bottomFlywheelSim = new FlywheelSim(m_bottomFlywheelPlant, DCMotor.getNEO(1),
            1 / Constants.OutakeConstants.flywheelGearRatio);
    ...
    public void setIndividualFlywheelSpeeds(double topWheelSpeed, double bottomWheelSpeed) {
        ...
        if (Robot.isReal()) {
            ...
            m_topLoop.setNextR(VecBuilder.fill(Units.rotationsPerMinuteToRadiansPerSecond(topWheelSpeed))); // because
                                                                                                            // it uses
                                                                                                            // radians..?
