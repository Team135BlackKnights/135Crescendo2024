...
//Heads up! It's unimportant, but if the driver shoots a note in sim, picks up a note BEFORE the shot one is at the amp, and then attempts to shoot the new note BEFORE the old shot one is at the amp, it will desync
//the array by 1, which will crash the program at the last note on the field. I'm not fixing this, because it'd require a lot of time, and is a VERY one-off scenario.
//To be exact, the "one-off scenario" is TWO NOTE CYCLE IN ONE SECOND. It doesn't happen. Technically, it could be fixed by upping shot speed, but we like to see the notes fly.
public class SimShootNote {
  ...
  public static void intake(int index){
    ...
    Commands.defer(
      () -> {
        ...
        final Pose3d endPose = new Pose3d(robotPoseSupplier.get()).transformBy(launcherTransform); 
        final double duration = startPose.getTranslation().getDistance(endPose.getTranslation()) / intakeSpeed;
  ...
  public static void updateStates(){
    Pose3d[] translator = new Pose3d[currentNotes.size()]; 
    launcherTransform =  new Transform3d( 0.3, 0, 0.36, new Rotation3d(0, -Units.degreesToRadians(IntakeS.getDistance()-Constants.IntakeConstants.intakeOffset+8), 0.0));
    heldNotePos = new Pose3d(robotPoseSupplier.get()).transformBy(launcherTransform);
    for (int i = 0; i < currentNotes.size(); i++){
      if (i == 0 && hasNote){
                    ...
                    return Commands.run(
                            () -> {
                              if (SimShootNote.firstOutake && indexChanger == 2){
                                currentNotes.remove(0);
                                currentNotes.add(currentNotes.size()-1,startPose.interpolate(endPose, timer.get() / duration));
                                ...
                                if (indexChanger == 2){
                                  ...
                                  currentNotes.add(currentNotes.size(),startPose.interpolate(endPose, timer.get() / duration));
                                }
                            }})
