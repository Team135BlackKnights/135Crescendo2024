...
public class AutonIntake extends Command {
    ...
    public static double closerAngleToZero(double angle) {
        ...
        return (Math.abs(normalizedAngle) <= 180) ? normalizedAngle : -normalizedAngle;
    }
    ...
    public void execute() {
        ...
        if (Robot.isSimulation()){
            ...
            //THESE ARE IN M E T E R S 
                ...
                ty = targetNoteLocation.getY() - currentPose.getY();// -0.307975;
        ...
        if (Robot.isReal()){
            ...
            SmartDashboard.putNumber("SIMDISTANCE ", Units.metersToInches(distance));
            if (distance <= 4.5){
                close = true;
            }
           }
        if (tv == false && loaded==false && close == false) {
            ...
            double moveSpeed =  Constants.DriveConstants.kMaxSpeedMetersPerSecond * speedMapper(distance);
            
            double turnSpeed = Units.degreesToRadians(error)*IntakeS.kP;
